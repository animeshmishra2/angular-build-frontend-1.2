...
export class AddAttributeValueComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<AddAttributeValueComponent>,
    ...
    if (this.row?.idattribute) {
      this.lForm.controls["name"].setValue(this.row.value);
  ...
  onSubmit() {
    ...
    if (this.row.idattribute) {
      ...
      this.storeWareServ.updateAttributeValue(this.row?.idattribute,payload).subscribe((data: any) => {
        this.cancel(false);
        this.alertService.openSnackBar("Sucessfully Updated Attribute Value");
      },
        (error) => {
      ...
      this.storeWareServ.createAttributeValue(payload).subscribe((data: any) => {
        this.cancel(false);
        this.alertService.openSnackBar("Sucessfully Created Attribute Value");
      },
        error => {
          this.alertService.error(error);
          this.loading = false;
