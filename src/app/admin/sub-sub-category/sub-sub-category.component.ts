...
import { EditSubSubCategoryComponent } from './edit-sub-sub-category/edit-sub-sub-category.component';
...
export class SubSubCategoryComponent implements OnInit {
  ...
  displayedColumns: string[] = ['idsub_category', 'name','category','sub_category', 'status', 'action'];
  ...
  getAll() {
    this.loading = true;
    this.apiServ.get(AppSetting.ENDPOINTS.subSubCategory)
      .subscribe(
        data => {
          this.scats = data;
  ...
  editItem(row) {
    const dialogRef = this.dialog.open(EditSubSubCategoryComponent, {
      width: '80%',
      data: { data: row }
    });
